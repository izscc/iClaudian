...
describe('InputController - Message Queue', () => {
  ...
  describe('Sending messages', () => {
    ...
    it('should ignore external contexts when the user explicitly asks for this note only', async () => {
      const preparedRequests: any[] = [];
      const fileContextManager = {
        startSession: jest.fn(),
        getCurrentNotePath: jest.fn().mockReturnValue('notes/session.md'),
      ...
      inputEl = deps.getInputEl() as ReturnType<typeof createMockInputEl>;
      inputEl.value = '仅翻译整理这条笔记';
      controller = new InputController(deps);

      await controller.sendMessage();

      expect(preparedRequests[0].currentNotePath).toBe('notes/session.md');
      expect(preparedRequests[0].externalContextPaths).toBeUndefined();
    });
